...
1. **Find the global CLAUDE.md path.** Claude Code stores global config in `~/.claude/` on all platforms (`$HOME/.claude/` on Mac/Linux, `$env:USERPROFILE\.claude\` on Windows). Target file: `~/.claude/CLAUDE.md`.
2. **Check if the session-rules reference exists** in that file.
3. **If missing**, add this block to `~/.claude/CLAUDE.md` (create if it doesn't exist):
   ```markdown
   ## Session Rules
   ...
   ```
4. Confirm: "Session rules installed in ~/.claude/CLAUDE.md — will auto-load on future sessions."

If the reference already exists, skip silently and apply the rules.

**Sync model:** CLAUDE.md holds only the pointer. The skill file holds all logic. Update the skill → every machine gets the change automatically on next load.
